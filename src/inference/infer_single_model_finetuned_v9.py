"""
Finetuned inference script with v9 prompt (anti-overthinking + trust first impression).
Based on v8, with targeted fixes for regression patterns.
Supports batch processing for faster inference.

Usage (batch size 4):
    python infer_single_model_finetuned_v9.py \
        ...
        --output_dir outputs/v9_finetuned \
        --batch_size 4
...
def prepare_single_input(processor: Qwen3OmniMoeProcessor, sample: Dict):
    ...
    user_prompt = build_v9_user_prompt(sample["question"], sample["choices"])

    conversation = [
        {
            "role": "system",
            "content": [{"type": "text", "text": V9_SYSTEM_PROMPT}],
        },
        {
            "role": "user",
            "content": [
